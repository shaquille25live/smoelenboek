...
class SecretaresseController  
{
    ...
    */
       ...
       switch($result)
            {
                ...
                case REQUEST_NOTHING_CHANGED:
                    ...
                    break;
                  case REQUEST_FAILURE_DATA_INVALID:
                    $this->view->set("boodschap","nieuwe wachtwoord niet identiek of oude wachtwoord fout. Poging opnieuw!");
                    break;
                case REQUEST_FAILURE_DATA_INCOMPLETE:
    ...
    private function boekenAction()
    {
        $boeken = $this->model->getBoeken();
        $this->view->set('boeken',$boeken);
        $gebruiker = $this->model->getGebruiker();
        $this->view->set('gebruiker',$gebruiker);
        if($this->model->isPostLeeg())
        {
           $this->view->set("boodschap","Vul gegevens in van een nieuw boek");          
        }
        else
        {   
            $result=$this->model->addBoek();
            switch($result)
            {
               
                case REQUEST_FAILURE_DATA_INCOMPLETE:
                    $this->view->set("boodschap", "Boek is niet toegevoegd. Niet alle vereiste data ingevuld.");  
                    $this->view->set('form_data',$_POST);
                    break;
                case REQUEST_FAILURE_DATA_INVALID:
                    $this->view->set("boodschap", "fout opgetreden, Boek niet toegevoegd");  
                    $this->view->set('form_data',$_POST);
                    break;
                case REQUEST_SUCCESS:
                    $this->view->set("boodschap", "Boek is toegevoegd"); 
                    $this->forward("beheer");
                    break;  
            }  
        }
